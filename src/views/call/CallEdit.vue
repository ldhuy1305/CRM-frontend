...
const fetchCall = async () => {
  try {
    ...
    // Map call data to form
    form.title = call.title
    form.description = call.description
    form.call_type = call.call_type.id
    form.call_purpose = call.call_purpose?.id || null
    form.call_result = call.call_result?.id || null
    form.call_owner = call.call_owner.id
    form.related_deal = call.related_deal?.id || null
    form.related_account = call.related_account?.id || null
    form.lead = call.lead?.id || null
    form.contact = call.contact?.id || null
    form.start_time = formatDateTimeLocal(call.start_time) // Format the date here
    form.duration = call.duration
    form.call_agenda = call.call_agenda
    form.voice_recording = call.voice_recording
    form.is_remind = call.is_remind
  } catch (error) {
...
const handleSubmit = async () => {
  if (validateForm()) {
    try {
      ...
      await callRepository.update(id, payload)
...
onMounted(async () => {
  isLoading.value = true
  await Promise.all([fetchCall(), fetchDropdownData()])
  isLoading.value = false
})
</script>
