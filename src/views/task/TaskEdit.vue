<template>
  <div class="page-container">
    ...
    <form v-if="task" @submit.prevent="handleSave" class="form-container">
...
const fetchTask = async () => {
  try {
    ...
    form.task_owner = task.value.task_owner?.id || 0
    form.due_date = task.value.due_date || ''
    form.status = task.value.status?.id || 0
    form.priority = task.value.priority?.id || 0
    form.contact = task.value.contact?.id || null
    form.related_account = task.value.related_account?.id || null
    form.related_deal = task.value.related_deal?.id || null
    form.is_remind = task.value.is_remind
    form.is_repeat = task.value.is_repeat
...
const handleSave = async () => {
  if (validateForm()) {
    try {
      isLoading.value = true
      await taskRepository.update(id, form)
